...
class GetTagHydrate(Resource):
    ...
    def get(self, tag_id):
        ...
        req += ' RETURN a.annotation_id AS annotation_id, a.timestamp AS annotation_timestamp, p.post_id as post_id, p.title as post_title, p.timestamp as post_timestamp, u.user_id as user_id, u.name as user_name'
        result = neo4j.query_neo4j(req)
        annotations_posts = []
        ...
        req += ' RETURN a.annotation_id AS annotation_id, a.timestamp AS annotation_timestamp, c.comment_id as comment_id, c.title as comment_title, c.timestamp as comment_timestamp, u.user_id as user_id, u.name as user_name'
        result = neo4j.query_neo4j(req)
